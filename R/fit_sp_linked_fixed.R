...
#' @export
fit_sp_linked_fixed <- function(data, b_col, sp_col, p=1, cov_col){
  ...
  # Compile TMB model
  dir.create(rundir)
  file.copy(from=file.path(tmbdir, "pella_linked_fixed.cpp"), to=file.path(rundir, "pella_linked_fixed.cpp"), overwrite=FALSE)
  on.exit(setwd(origdir),add=TRUE)
  setwd(rundir)
  compile("pella_linked_fixed.cpp")
  dyn.load(TMB::dynlib(file.path(rundir, "pella_linked_fixed")))
  ...
  # Initialize model
  model <- TMB::MakeADFun(data=input.data, parameters=params, DLL="pella_linked_fixed")
  ...
  # Package data
  output <- list(data=data, fit=fit, sd=sd, p=p, cov_effect="fixed")
