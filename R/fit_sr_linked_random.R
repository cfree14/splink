...
fit_sr_linked_random <- function(data, b_col, r_col, cov_col, type){
  ...
  # Compile TMB model
  dir.create(rundir)
  file.copy(from=file.path(tmbdir, "srmodel_linked_random.cpp"), to=file.path(rundir, "srmodel_linked_random.cpp"), overwrite=FALSE)
  on.exit(setwd(origdir), add=TRUE)
  setwd(rundir)
  compile("srmodel_linked_random.cpp")
  dyn.load(TMB::dynlib(file.path(rundir, "srmodel_linked_random")))
